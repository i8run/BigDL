...
class ReorderMemory(inputFormat: MemoryData, outputFormat: MemoryData,
  ...
  override private[mkldnn] def initFwdPrimitives(inputs: Array[MemoryData], phase: Phase) = {
    ...
    val fwdReorderPrimDesc = if (needNotAttr) {
      ...
      if (realOutput(0).scales == null || !realOutput(0).scales.nonEmpty) {
        if (realOutput(0).dataType == DataType.F32) {
          realOutput(0).setScales(realInput(0).scales.map(1.0f / _))
        } else {
    ...
    _inputFormats = realInput
    _outputFormats = realOutput
